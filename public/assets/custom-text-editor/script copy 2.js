...
function insertVideo() {
  ...
  showModal("Insert Video", modalHtml, () => {
    const upload = document.querySelector("input[name='videoType'][value='upload']:checked");
    const mp4Url = document.querySelector("input[name='videoType'][value='url']:checked");
    const youtubeChoice = document.querySelector("input[name='videoType'][value='youtube']:checked");
    ...
    // 2. MP4 URL
    else if (mp4Url && mp4TextUrl && mp4TextUrl.startsWith("http") && mp4TextUrl.includes(".mp4")) {
      const editor = document.getElementById("editor");
      editor.focus();
      restoreSelection();

      const videoHTML = `<div style="margin:8px 0;text-align:center;"><video controls style="max-width:100%;height:auto;"><source src="${mp4TextUrl}" type="video/mp4"></video></div>`;
      if (document.queryCommandSupported("insertHTML")) {
    ...
    // 3. YouTube URL
    else if (youtubeChoice && youtubeRaw && youtubeRaw.trim().length > 0) {
      const editor = document.getElementById("editor");
      editor.focus();
      restoreSelection();
      ...
      const youtubeHTML = `<div style="margin:8px 0;text-align:center;"><iframe width="560" height="315" src="https://www.youtube.com/embed/${id}" frameborder="0" allowfullscreen></iframe></div>`;
      if (document.queryCommandSupported("insertHTML")) {
        document.execCommand("insertHTML", false, youtubeHTML);
      } else {
        pasteHtmlAtCaret(youtubeHTML);
      }
    }
    else {
